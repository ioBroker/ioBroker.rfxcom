const subTypes = ['WEIGHT_UNUSED', 'WEIGHT1', 'WEIGHT2'];
...
function Weight(comm, options, log) {
    ...
    if (!subTypes.includes(options.subtype)) {
        return log && log.warn('No subtype or invalid subtype is defined. Please define on of ' + subTypes.join(', '));
    }
    this.commands = [];
    ...
    this.sendCommand = function (cmd, level, callback) {
        ...
        typeof callback === 'function' && callback('Command not supported');
    };
